---
title: "EC and WGU DAACS Binary SuperLearners"
...
The purpose of this document is to generate SuperLearner objects for all DAACS binary variables for EC and WGU.
...
A couple variables
```{r}
#Inner cross-validation folds for SuperLearner
inner_cv <- 5
...
ec_training_rows <- sample(nrow(ec_tidy_complete), training_fraction * nrow(ec_tidy_complete), replace = FALSE)
ec_daacs_trn <- ec_tidy_complete[ec_training_rows, ec_daacs_vars]
ec_daacs_tst <- ec_tidy_complete[-ec_training_rows, ec_daacs_vars]

wgu_training_rows <- sample(nrow(wgu_tidy), training_fraction * nrow(wgu_tidy), replace = FALSE)
wgu_daacs_trn <- wgu_tidy[wgu_training_rows, wgu_daacs_vars]
wgu_daacs_tst <- wgu_tidy[-wgu_training_rows, wgu_daacs_vars]
```
...
```{r}
ec_daacs_trn <- mod_mtx(ec_daacs_trn)
ec_daacs_tst <- mod_mtx(ec_daacs_tst)
```

```{r}
wgu_daacs_trn <- mod_mtx(wgu_daacs_trn)
wgu_daacs_tst <- mod_mtx(wgu_daacs_tst)
```
...
## Model ec_daacs_Retained

```{r}
model_title <- "ec_daacs_Retained"
explanatory_training_set <- ec_daacs_trn
...
set.seed(345, "L'Ecuyer-CMRG")
assign(model_title,
       SuperLearner(Y = response_training_set,
                            X = explanatory_training_set,
                            family = binomial(),
                            method = "method.AUC",
                            cvControl = list(V = inner_cv),
                            SL.library = SLlib_bin))
```

## Model ec_daacs_SuccessTerm1

```{r}
model_title <- "ec_daacs_SuccessTerm1"
explanatory_training_set <- ec_daacs_trn
...
## Model ec_daacs_SuccessTerm2

```{r}
model_title <- "ec_daacs_SuccessTerm2"
explanatory_training_set <- ec_daacs_trn
...
## Model ec_daacs_CreditRatio_Term1

```{r}
model_title <- "ec_daacs_CreditRatio_Term1"
explanatory_training_set <- ec_daacs_trn
...
## Model ec_daacs_CreditRatio_Term2

```{r}
model_title <- "ec_daacs_CreditRatio_Term2"
explanatory_training_set <- ec_daacs_trn
...
## Model wgu_daacs_Retained

```{r}
model_title <- "wgu_daacs_Retained"
explanatory_training_set <- wgu_daacs_trn
...
## Model wgu_daacs_OnTime_Term1

```{r}
model_title <- "wgu_daacs_OnTime_Term1"
explanatory_training_set <- wgu_daacs_trn
...
## Model wgu_daacs_OnTime_Term2

```{r}
model_title <- "wgu_daacs_OnTime_Term2"
explanatory_training_set <- wgu_daacs_trn
...
## Model wgu_daacs_CreditRatio_Term1

```{r}
model_title <- "wgu_daacs_CreditRatio_Term1"
explanatory_training_set <- wgu_daacs_trn
...
## Model wgu_daacs_CreditRatio_Term2

```{r}
model_title <- "wgu_daacs_CreditRatio_Term2"
explanatory_training_set <- wgu_daacs_trn
